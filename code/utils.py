...
def load_data(preprocessor=True):
    ...
    df_avis = df_avis.drop_duplicates(subset=['author','title','note','comment','date_published'])
    df_avis['date_published'] = pd.to_datetime(df_avis['date_published'])
    df_avis = df_avis.sort_values(['author', 'title', 'comment' ,'date_published'])
    df_avis = df_avis.groupby(['author', 'title']).agg({'note': 'last', 'comment': 'last' ,'date_published': 'last'}).reset_index()
